 ...
 *  USA
 *
 *  $Id: m_message.c 117 2010-01-21 15:53:04Z karel.tuma $
 */
...
DECLARE_MODULE_AV2(message, modinit, moddeinit, message_clist, NULL, NULL, "$Revision: 117 $");
	...
	if(GlobalSetOptions.floodcount && MyClient(source_p))
	{
		if((chptr->first_received_message_time + 1) < rb_current_time())
		{
			...
			if(chptr->received_number_of_privmsgs <= 0)
			{
				...
				SetChannelUnflooded(chptr);
			}
		}

		if((chptr->received_number_of_privmsgs >= GlobalSetOptions.floodcount)
		   || IsChannelFlooded(chptr))
		{
			if(!IsChannelFlooded(chptr))
			{
				sendto_realops_flags(UMODE_BOTS, L_ALL,
						     "Possible Flooder %s[%s@%s] on %s target: %s",
						     source_p->name, source_p->username,
